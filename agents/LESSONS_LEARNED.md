...
- **`cargo fmt` after large edits, not before.** After writing 1400-line generated tool file via sub-agent, `cargo fmt --check` blocked CI because `#[schemars(description = "...long string...")]` attributes needed wrapping. Always run `cargo fmt` as the last step after a large code generation pass, before code review.

- **Renamed MCP fields with silent defaults create migration hazard.** `query` → `search_query` and `limit` → `top_k` on `memory.retrieve_memory_records` are unknown-field silently-dropped by serde — callers using old field names get empty results or default limits with no error. Document in CHANGELOG with a scoped grep; consider adding a `#[serde(alias = "...")]` for one release cycle.
